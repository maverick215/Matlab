...
papOutpPredbyNN = xlsread('mgpapoutpRED100.xls'); % Predicted output for ART for 598 samples (~600)
disp('Sample size in original waveform =')
length(papOutpPredbyNN)
ts1 = papOutpPredbyNN;
...
yi_ts1 = interp1(samples./2, ts1, 1.5:1:length(ts1)); % Samples need to be set back to actual #samples; 1.5:1: ensures we predict the right middle values by interpolation
...
err_ts1 = gsubtract(ts1x2, papOutpPredbyNN);
...
err_ts2 = gsubtract(filledOS, papOutpPredbyNN);
...
originalSamples = papOutpPredbyNN;
...
yi_2 = interp1(sampleInstants, reducedSamples, 2:4:length(originalSamples));
...
yi_3 = interp1(sampleInstants, reducedSamples, 3:4:length(originalSamples));
% The last element in this series containing the result of interpolation has to be removed if we were required to extend the
% original series earlier
...
yi_4 = interp1(sampleInstants, reducedSamples, 4:4:length(originalSamples));
% The last element in this series containing the result of interpolation has to be removed if we were required to extend the
% original series earlier
...
err_ts3 = gsubtract(filledOS1, papOutpPredbyNN);
...
yi_2 = interp1(sampleInstants, reducedSamples, 2:5:length(originalSamples));
...
yi_3 = interp1(sampleInstants, reducedSamples, 3:5:length(originalSamples));
% The last element in this series containing the result of interpolation has to be removed if we were required to extend the
% original series earlier
...
yi_4 = interp1(sampleInstants, reducedSamples, 4:5:length(originalSamples));
% The last element in this series containing the result of interpolation has to be removed if we were required to extend the
% original series earlier
...
yi_5 = interp1(sampleInstants, reducedSamples, 5:5:length(originalSamples));
% The last element in this series containing the result of interpolation has to be removed if we were required to extend the
% original series earlier
...
err_ts4 = gsubtract(filledOS2(1:length(papOutpPredbyNN)), papOutpPredbyNN);
...
disp('--------------------')
disp('Signal minimum = ') 
disp(min(papOutpPredbyNN))
disp('Signal maximum = ') 
disp(max(papOutpPredbyNN))
...
disp(max(err_ts1(20:length(err_ts1)-5))) 
disp(max(err_ts2(20:length(err_ts2)-5))) 
disp(max(err_ts3(20:length(err_ts3)-5))) 
disp(max(err_ts4(20:length(err_ts4)-5)))
disp('--------------------')
